...
function infile(book::Vector{RegexMatch})
    open("./arb/orderbooks/BNBBTC.csv", "w+") do io
...
HTTP.WebSockets.open(string(BN.BINANCE_API_WS, lowercase("BNBBTC"), string("@depth", 5, "@100ms")); verbose=false) do io
    while !eof(io)
        ...
        try
            ...
            @error("Не удалость записать ордербук BNBBTC") # "julia > book &". При это команде, эта строка выбьется в консоль
            println("Не удалость записать ордербук BNBBTC") # А эта в файл.
        end
    end
end
